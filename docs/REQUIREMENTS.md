...
- **卡片内容**：
  ...
  - **7 天窗口健康配额线**：7 天额度按天均摊（每天 1/7 ≈ 14.28%），进度条上叠加一条按「当天目标配额」定位的半透明灰刻度线：窗口起点 = 重置时间 − 7 天，已完整过整天数 d = floor(已过天数)，位置 = d/7。第 1 天 14.28%、第 2 天 28.57% … 第 6 天 85.71%；**最后一天（d=6）封顶停在 85.71%，不画到 100%**（进度条末端无意义）；第 0 天（刚重置）不画线。用量超过该线即超前消耗、挤占后续份额。线与进度条同高内嵌（不凸出、不改变原进度条高度）、约 2px、圆头；颜色为半透明灰（约 75% 不透明），浅/深主题下均柔和可见。Widget 每次刷新时按当前时间更新；文本兜底渲染时以「▎」刻度字符标记。
